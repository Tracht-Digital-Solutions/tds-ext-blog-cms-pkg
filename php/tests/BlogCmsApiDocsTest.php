 ...
 */
final class BlogCmsApiDocsTest extends TestCase
{
    ...
    private static function mountedRoutes(): array
    {
        ...
        $registry = new ModuleRegistry([new BlogCmsModule()]);
    ...
    public function testEveryEntryIsWellFormed(): void
    {
        $permissions = array_map(
            ...
            (new BlogCmsModule())->permissions(),
        );

        foreach ((new BlogCmsModule())->apiDocs() as $doc) {
    ...
    public function testEverySiteKeyPrefixStillCoversAMountedRoute(): void
    {
        ...
        foreach ((new BlogCmsModule())->siteKeyRoutes() as $prefix) {
            $covers = false;
     ...
     *
     * `ModuleRegistry` throws on an `/admin` prefix, but only for a prefix that
     * *starts* with it. This catches the other direction — a prefix so broad
     * that an admin route happens to fall under it — which would turn a CI
